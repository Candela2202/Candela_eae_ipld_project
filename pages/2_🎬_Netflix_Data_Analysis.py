...
split_list_countries = countries_str.split(", ")

#print(split_list_countries)

unique_list = []

for country in split_list_countries:
    if country not in unique_list and country != ""  and "," not in country:
       unique_list.append(country)

print(sorted(unique_list))

n_countries = len(unique_list)
...
data_year = movies_df.loc[movies_df['release_year'] == year]
top_10_countries = pd.Series(data_year["country"].value_counts().head(10))
...
movies_df['movies_minutes'] = movies_df['duration'].apply(lambda x: int(x.strip("min")) if "min" in x else None)
grouped_series = pd.Series(movies_df.groupby("release_year")["movies_minutes"].mean())
